...
DWORD __stdcall CClient::_SendRecvWorkThreadFunc(LPVOID lpParam) {
  ...
  for (int i = 1; i < 4; i++) {
    ...
    sprintf(szTemp, "%d's time Message: %s\n", i,pParams->szBuffer);
    ...
    TRACE(_T("发送第%d次信息成功.\n", i));
    pClient->ShowMessage(_T("向服务器发送信息成功: %s"), szTemp);

    // 3s后再发下一条信息
    Sleep(3000);
  }
  if (pParams->nThreadID == pClient->m_nThreads)
    pClient->ShowMessage(_T("测试并发%d个线程完毕!"), pClient->m_nThreads);
  return 0;
}
...
bool CClient::EstablishConnections() {
  ...
  for (int i = 0; i < m_nThreads; i++) {
    ...
    sprintf(m_pThreadParamSendRecv[i].szBuffer, " NO.%d Thread Send Data: %s", 
		    m_pThreadParamSendRecv[i].nThreadID,
...
bool CClient::ConnectToServer(SOCKET *pSock, CString strServerName, int nPort) {
  ...
  *pSock = socket(AF_INET, SOCK_STREAM, 0);
  ...
  char strHost[50];
  sprintf(strHost, "%s", strServerName);
  serverHost = gethostbyname(strHost);
  if (serverHost == NULL) {
